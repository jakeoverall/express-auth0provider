...
function fetchUserInfo(token = '') {
  ...
  if (!token) {
    throw new BadRequest("Invalid token: no token provided");
  }
 ...
 */
  ...
  static getAuthorizedUserInfo(req, res, next) {
    ...
    if (!userInfo) {
      fetchUserInfo(req.headers.authorization)
        .then((userInfo) => {
          userInfo = Auth0Provider.stripUrlBasedClaims(userInfo);
 ...
 */
  static async getUserInfoFromBearerToken(token) {
    if (!token) {
      throw new BadRequest("Invalid token: no token provided");
    }
    ...
    if (!sub) {
      throw new BadRequest("Invalid token: no subject found in claims, please check your token");
    }
    let userInfo = userInfoCache.get(sub);
    if (userInfo) {
    ...
    try {
      ...
      console.error(e);
      throw new Unauthorized("Invalid token: no user found in claims, please check your token");
    }
  }



  static getIdentity(token = "") {
    const identity = parseJWT(token);
    return identity;
  }
