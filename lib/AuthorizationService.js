...
export class Auth0Provider {
   ...
   * @param {{ domain: string; clientId: string; audience: string; dontStrip?:boolean }} config
   */
  configure({ domain, clientId, audience, dontStrip = false }) {
    if (!domain || !clientId || !audience) {
      throw new Error("[INVALID AUTH0 CONFIG]");
    }
    authConfig = { domain, clientId, audience, dontStrip };
  }
